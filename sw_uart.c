...
unsigned char ReadUART(void)
{
    ...
    while (bitcount--){
            DelayRXBitUART();
    //	STATUSbits.C = PORTBbits.RB6 ? 1:0;
    //	uartdata=uartdata>>1 | uartdata<<7;
            temp = PORTBbits.RB6 ? 0x80:0x00;
            uartdata=uartdata>>1 | temp;

            // note: als dit niet werkt dan 8 globals vullen met individuele bitwaarden
            // OF registreer aantal wisselingen oid...
    }
    return uartdata;
}
